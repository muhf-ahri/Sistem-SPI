...
<div class="card">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold mb-0 text-primary">Formulir Edit Pemeriksaan</h5>
    </div>
    <div class="card-body">
        @if ($errors->any())
        ...
        <form method="POST" action="{{ route('inspections.update', $inspection) }}">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('result') is-invalid @enderror" id="result" name="result" required>
                        <option value="satisfactory" {{ old('result', $inspection->result) == 'satisfactory' ? 'selected' : '' }}>Satisfactory (Memuaskan)</option>
                        <option value="needs_improvement" {{ old('result', $inspection->result) == 'needs_improvement' ? 'selected' : '' }}>Needs Improvement (Perlu Perbaikan)</option>
                        <option value="non_conformity" {{ old('result', $inspection->result) == 'non_conformity' ? 'selected' : '' }}>Non Conformity (Ketidaksesuaian)</option>
                    </select>
                    @error('result')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('summary') is-invalid @enderror" id="summary" name="summary" rows="4" required>{{ old('summary', $inspection->summary) }}</textarea>
                    @error('summary')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="4">{{ old('notes', $inspection->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                <a href="{{ route('inspections.show', $inspection) }}" class="btn btn-secondary">Batal</a>
