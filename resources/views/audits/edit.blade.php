...
@section('content')
<div class="mb-4">
    <h1 class="h3 fw-bold mb-0">Edit Pengawasan</h1>
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route('audit-plans.update', $auditPlan) }}">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('division_id') is-invalid @enderror" id="division_id" name="division_id" required>
                        ...
                        @foreach($divisions as $id => $name)
                            <option value="{{ $id }}" {{ old('division_id', $auditPlan->division_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                    @error('division_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('audit_type_id') is-invalid @enderror" id="audit_type_id" name="audit_type_id" required>
                        ...
                        @foreach($auditTypes as $id => $name)
                            <option value="{{ $id }}" {{ old('audit_type_id', $auditPlan->audit_type_id) == $id ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                    @error('audit_type_id')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control @error('start_date') is-invalid @enderror" id="start_date" name="start_date" value="{{ old('start_date', $auditPlan->start_date) }}" required>
                    @error('start_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control @error('end_date') is-invalid @enderror" id="end_date" name="end_date" value="{{ old('end_date', $auditPlan->end_date) }}" required>
                    @error('end_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('auditor_ids') is-invalid @enderror" id="auditor_ids" name="auditor_ids[]" multiple style="height: 100px;">
                        @foreach($auditors as $id => $name)
                            <option value="{{ $id }}" {{ is_array(old('auditor_ids', $selectedAuditors)) && in_array($id, old('auditor_ids', $selectedAuditors)) ? 'selected' : '' }}>{{ $name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4">{{ old('description', $auditPlan->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
